...
function Stars({ rating, muted = false }: { rating: number; muted?: boolean }) {
  return (
    <span aria-label={`${rating} üzerinden 5 yıldız`} className={`text-sm tracking-[0.16em] ${muted ? 'text-cream/25' : 'text-gold'}`}>
...
export function ProductReviews({ productId, productSlug, productName, reviews }: ProductReviewsProps) {
  ...
  const submit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!response.ok) {
        setStatus(data?.error || 'Yorum gönderilemedi.')
        return
      }
      setBody('')
      setTitle('')
      setShowForm(false)
      setStatus('Teşekkürler. Yorumunuz yönetici onayından sonra yayınlanacak.')
    } finally {
      setSubmitting(false)
    }
  }
  ...
  return (
    <section className="mt-20 border-t border-gold/20 pt-12 md:pt-16" aria-labelledby="reviews-heading">
      <div className="flex flex-col gap-5 border-b border-cream/10 pb-8 md:flex-row md:items-end md:justify-between">
        <div>
          <p className="text-xs font-medium uppercase tracking-[0.18em] text-gold">Müşteri deneyimleri</p>
          <h2 id="reviews-heading" className="mt-2 text-3xl font-serif font-bold text-cream md:text-4xl">{productName} yorumları</h2>
          <p className="mt-2 text-sm text-cream/55">Yalnızca satın alma kaydı doğrulanan üyelerin yorumları yayınlanır.</p>
        </div>
        {signedIn === false ? (
          <Link
      ...
      {showForm && signedIn && (
        <form onSubmit={submit} className="mt-7 rounded-2xl border border-gold/25 bg-[#0f0e0c] p-5 shadow-[0_20px_48px_-38px_rgba(0,0,0,0.92)] md:p-6">
          <div className="flex flex-wrap items-center gap-2">
            <span className="mr-2 text-sm text-cream/70">Puanınız</span>
            {[1, 2, 3, 4, 5].map((value) => (
              <button
                key={value}
                type="button"
          ...
          <div className="mt-5 grid gap-4">
            <input
              ...
              maxLength={120}
              placeholder="Kısa başlık (isteğe bağlı)"
          ...
          <div className="mt-5 flex flex-wrap items-center gap-4">
            ...
            <p className="text-xs text-cream/45">Yorumunuz görünmeden önce kalite kontrolünden geçer.</p>
          </div>
        </form>
      )}
      ...
      {reviews.length ? (
          ...
          {reviews.map((review) => (
            <article key={review.id} className="rounded-2xl border border-cream/10 bg-[#0f0e0c] p-5 shadow-[0_18px_46px_-34px_rgba(0,0,0,0.95)]">
              <div className="flex items-start justify-between gap-4">
                ...
                {review.verified_purchase && <span className="shrink-0 text-[10px] font-medium uppercase tracking-[0.12em] text-gold">Doğrulanmış alım</span>}
              </div>
        ...
        <p className="mt-8 rounded-2xl border border-dashed border-cream/15 px-5 py-8 text-sm text-cream/50">
          Bu ürün için henüz onaylanmış yorum yok. İlk deneyimi paylaşan siz olun.
        </p>
      )}
    </section>
  )
